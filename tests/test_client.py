...
def default_test_state(data_file):
    create_test_data(
        data_file, ["test string 1", "test string 2"]
    )
...
def test_client_query_success(config_file, data_file, default_test_state):
    server_process, port, _, _ = server_factory(config_file)
...
def test_client_ssl_no_certificate(config_file):
    server_process, port, _, _ = server_factory(config_file, ssl_enabled=True)
    config = ClientConfig(
